<template>
  <div>
    <span @click="open = true" class="cursor-pointer">
      <FontAwesomeIcon class="edit-icon" :icon="icon" />
    </span>
              ...
              <DialogPanel
                ...
                <div class="bg-background px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                  <div class="sm:flex sm:items-start">
                    <div class="mt-3 full-width text-center sm:mt-0 sm:ml-4 sm:text-left">
                      <DialogTitle as="h3" class="text-base font-semibold text-white">{{ title }} {{ t('registry') }}
                      </DialogTitle>
                ...
                <div class="bg-soft px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                  <button type="button"
                    ...
                    @click="open = false" ref="cancelButtonRef">{{ t('no') }}</button>
                  <button type="button"
                    ...
                    @click="() => {
                      confirmAction(rowId, status === 'N' ? 'S' : 'N')
                      open = false
                    }" ref="cancelButtonRef">{{ t('yes') }}</button>
                </div>
              </DialogPanel>
            </TransitionChild>
          </div>
...
defineProps<{
  rowId: number,
  status: string,
  confirmAction: (rowId: number, status: string | undefined) => Promise<void>,
  msg: string,
  title: string,
  icon: IconDefinition
}>()
